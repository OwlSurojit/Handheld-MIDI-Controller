...
struct __attribute__((packed)) LatencyPacket {
    uint8_t type;  // 0xFF = latency check
    uint8_t mac[6];
...
void sendUdpPacket(const uint8_t *packet, size_t size) {
...
void sendLatencyPacket(uint32_t ts) {
    ...
    sendUdpPacket((const uint8_t *)&pkt, sizeof(pkt));
...
void handleEchoPacket() {
    ...
    if (packetSize == sizeof(LatencyPacket)) {
        ...
        if (echo.type == 0xFF && memcmp(echo.mac, MAC_ADDR, 6) == 0) {
...
void loop() {
    ...
    // Send latency check packet every 5 seconds
